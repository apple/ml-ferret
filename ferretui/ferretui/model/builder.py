...
def load_pretrained_model(model_path, model_base, model_name, use_safetensors=False, load_8bit=False, load_4bit=False, device_map="auto", device="cuda", use_flash_attn=False, **kwargs):
    ...
    if 'ferret' in model_name.lower():
        # Load LLaVA model
        if 'lora' in model_name.lower() and model_base is None:
            warnings.warn('There is `lora` in model name but no `model_base` is provided. If you are loading a LoRA model, please provide the `model_base` argument. Detailed instruction: https://github.com/haotian-liu/LLaVA#launch-a-model-worker-lora-weights-unmerged.')
        if 'lora' in model_name.lower() and model_base is not None:
            from ferretui.model.language_model.ferret_llama import FerretConfig
            lora_cfg_pretrained = FerretConfig.from_pretrained(model_path)
            ...
            if 'mpt' in model_name.lower():
                ...
                model = FerretMptForCausalLM.from_pretrained(model_base, low_cpu_mem_usage=True, config=cfg_pretrained, **kwargs)
            else:
                tokenizer = AutoTokenizer.from_pretrained(model_base, use_fast=False)
                cfg_pretrained = AutoConfig.from_pretrained(model_path)
                model = FerretLlamaForCausalLM.from_pretrained(model_base, low_cpu_mem_usage=True, config=cfg_pretrained, use_safetensors=use_safetensors, **kwargs)
            ...
            if 'mpt' in model_name.lower():
                ...
                model = FerretMptForCausalLM.from_pretrained(model_path, low_cpu_mem_usage=True, **kwargs)
